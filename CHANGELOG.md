...
# 2026.9.19 — KRÉTA Secure architecture

- Added strict HTTPS destination and redirect validation.
- Added GUI-based transient 2FA/recovery-code support.
- Removed persistent password storage and legacy raw JSON/profile entities.
- Added per-feature polling, read-only messages and absences, native summary
  entities, privacy-safe diagnostics and manual-refresh throttling.
- Added restart-safe hashed event baselines and new-item/timetable events.
- Added audit, network, privacy, security, upstream and third-party documents.
