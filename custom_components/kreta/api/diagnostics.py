...
def sanitize_form_data(data: dict[str, Any]) -> dict[str, str]:
    ...
    """
    return {
        k: _REDACTED if k.lower() in _SENSITIVE_REQUEST_FIELDS else str(v) for k, v in data.items()
    }
...
def sanitize_redirect_url(url: str) -> str:
    ...
    try:
        ...
        sanitized_params = {
            k: [_REDACTED] if k in _SENSITIVE_URL_PARAMS else v for k, v in params.items()
        }
        new_query = urlencode(
        ...
        *redirect_location* are sanitized before storage.
        ...
        self._steps.append(
            _TraceStep(
                ...
                method=method,
                url=url,
                sanitized_request_data=(sanitize_form_data(request_data) if request_data else None),
                response_status=response_status,
                response_body=(sanitize_response_body(response_body) if response_body else None),
                redirect_location=(
                    sanitize_redirect_url(redirect_location) if redirect_location else None
                ),
